...
the core scheduling algorithm, and the considerations that allow for nested and
repeatedly polled futures. Section
...
in the DAG. Parsimonious scheduling achieves this by having each worker maintains its own deque of nodes
that represent work to be executed, and having workers continuously pops nodes
off and executing them. Upon completion of a node execution, the node may enable
zero, one, or two child nodes. If zero nodes are enabled, it attempts to pop off
its deque. If one node is enabled, it immediately executes the enabled node. If
two nodes are enabled, it pushes one of the nodes to its end of the deque and
executes the other node.
...
case of zero nodes, meaning a worker simply continues looking for work in its
...
\begin{itemize}
    \item \textbf{Active}: It is the designated active deque of a given worker
        thread (the worker thread pops, executes, and pushes nodes to this
        deque).
    ...
    \item \textbf{Resumable:} All nodes in the deque are ready, but is not being
        actively worked on by a worker thread. These nodes can be stolen by
        worker threads to execute.
...
operation. During scheduler execution, thieves select a victim worker thread
uniformly at random to steal from, and from within that victim's stealable set
uniformly at random select a victim deque. Stealable set operations are assumed
to take constant amortized time.
...
stealable set, so that worker threads can not fruitlessly try to steal from it. 
...
operations that are not directly related to executing work nodes. As such, the
callback is responsible for executing when completion of a blocked future is
detected, and rescheduling the future node for execution by a worker thread.
More concrete reasoning and explanation of how this occurs in practice is given
in section \ref{section:i/o_dispatch_and_processing}.
...
multiple wakers to wake up the same future. This can happen, for example, when
two futures are manually polled immediately after one another using the same
waker, like when using the \texttt{join!} macro \footnote{The \texttt{join!}
    macro can be used to create a future that concurrently executes two or more
...
futures.}. Because of this, it is vital that ProWS-R supports \textit{repeatedly
polled futures}. Fortunately, supporting this is trivial: all that is required
...
any actions. If, however, the deque is still suspended, the callback pushes
the future back to the bottom of the deque, transitions it to Resumable, and
adds it to a random worker thread's stealable set if the deque is not already in
one. Doing this the callback makes the now-resumable future ready to be executed
by a worker thread again.
...
as Muggable, and possibly a new deque is created for the thieving worker thread
if it has none (which is the case when a blocked future is encountered on line
...
stealable deque given a victim worker thread stays uniform. This is called when
a deque has been removed from a worker thread \(v\)'s stealable set- it randomly
chooses another victim \(v^\prime\) and if \(v = v^\prime\) nothing is done,
otherwise a stealable deque is moved from \(v^\prime\) to \(v\) if \(v^\prime\)
has one.

\begin{algorithm}
\caption{Main Scheduling Loop ($w$ is the currently executing worker thread)}\label{alg:scheduling_loop}
\begin{algorithmic}[1]
